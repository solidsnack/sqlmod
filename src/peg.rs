                ...
                Matched(pos, _) => {
                    {
                        ...
                        match seq_res {
                            Matched(pos, _) => {
                                {
                                    let seq_res =
                                        parse_signature(input, state, pos);
                                    match seq_res {
                                        Matched(pos, signature) => {
                                            {
                                                let seq_res =
                                                    {
                                                        let mut repeat_pos =
...
fn parse_signature<'input>(input: &'input str, state: &mut ParseState<'input>,
                           pos: usize) -> RuleResult<Signature> {
    {
        let start_pos = pos;
        {
            ...
            match seq_res {
                Matched(pos, name) => {
                    {
                        let seq_res =
                            match parse_parenthesized(input, state, pos) {
                                Matched(newpos, value) => {
                                        ...
                                        Matched(pos, annotations) => {
                                            {
                                                ...
                                                Matched(pos,
                                                        {
                                                            let annotations =
                                                                annotations.unwrap_or(vec!()).into_iter();
                                                            let parenthesized =
                                                                parenthesized.into_iter();
                                                            let combined =
                                                                parenthesized.chain(annotations).map(|s|
                                                                                                         s.into());
                                                            Signature{name:
                                                                          name.into(),
                                                                      attributes:
...
fn parse_annotation<'input>(input: &'input str,
                            state: &mut ParseState<'input>, pos: usize)
 -> RuleResult<&'input str> {
    {
        let start_pos = pos;
        {
            let seq_res =
                {
                    let choice_res = parse_plain_word(input, state, pos);
                    match choice_res {
                        Matched(pos, value) => Matched(pos, value),
                        Failed => {
                            let choice_res =
                                {
                                    let seq_res =
                                        parse_plain_word(input, state, pos);
                                    match seq_res {
                                        Matched(pos, _) => {
                                            parse_balanced_parens(input,
                                                                  state, pos)
                                        }
                                        Failed => Failed,
                                    }
                            ...
                            match choice_res {
                                ...
                                Failed => {
                                    let choice_res =
                                        parse_balanced_parens(input, state,
                                                              pos);
                                    match choice_res {
                                        Matched(pos, value) =>
                                        Matched(pos, value),
...
fn parse_parenthesized<'input>(input: &'input str,
                               state: &mut ParseState<'input>, pos: usize)
 -> RuleResult<&'input str> {
    {
        let start_pos = pos;
        {
            let seq_res = parse_balanced_parens(input, state, pos);
            match seq_res {
                Matched(pos, _) => {
                    {
                        let match_str = &input[start_pos..pos];
...
fn parse_plain_word<'input>(input: &'input str,
                            state: &mut ParseState<'input>, pos: usize)
 -> RuleResult<()> {
    {
        let mut repeat_pos = pos;
...
fn parse_balanced_parens<'input>(input: &'input str,
                                 state: &mut ParseState<'input>, pos: usize)
 -> RuleResult<()> {
    {
...
fn parse_quoted<'input>(input: &'input str, state: &mut ParseState<'input>,
